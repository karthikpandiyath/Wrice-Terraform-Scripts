...
data "aws_vpc" "vpc_rds" {
  filter {
    name = "tag:Name"
    values = var.db_instance_singapore[*]["vpc_name"]
  }
}
...
data "aws_subnet_ids" "example" {
  count = length(var.db_instance_singapore[*]["subnet_name"])
  vpc_id = local.lcl_vpc_rds
  filter {
    name   = "tag:Name"
    values = flatten([var.db_instance_singapore[*]["subnet_name"]])
  }
...
resource "aws_db_subnet_group" "default" {
  ...
  name       = element(var.db_instance_singapore[*]["db_subnet_grp_name"],count.index)
